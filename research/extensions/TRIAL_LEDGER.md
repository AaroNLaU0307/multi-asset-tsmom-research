...
**Appended 2026-09-22 (CTA-EDGE-05 / F6 S3 run) — STATUS COLUMN UPDATED IN PLACE,
disclosed rather than hidden.** The `F-F6` status cell above previously read, at the
seal: *"NOTHING HAS RUN. `F6_FAMILY_STATUS = SEALED / NOT EXECUTED`.
`F6_PRIMARY_TRIAL_SPENT = NO`."* That sentence was true when written and is false now,
so the status column — and **only** the status column — was updated to record the
consumed trial, exactly as the `F-BENB` and `F-MMV` rows carry their executed state.
The declaring authority, membership, `m = 1` treatment and multiplicity text are
untouched, no row was deleted or reordered, and the superseded sentence is quoted here
verbatim so the change is auditable rather than silent.

The run was authorized by `F6-AUTH-0001`, committed before any target value was read,
and it spent the trial exactly once. **`N_trials` on the ETF panel remains NOT
ASSERTED** — `D-ETF-COUNT` stays `UNKNOWN_PENDING_AARON_DECISION`, and the sealed F6
